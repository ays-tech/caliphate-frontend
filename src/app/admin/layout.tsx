...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    <div className="flex flex-col h-full">
      <div className="px-5 py-5 border-b border-ink-800">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="font-display text-ivory text-xs tracking-[0.15em] leading-none">
              MAKHTABA
            </p>
            <p className="font-arabic text-gold-400 text-sm leading-none mt-0.5">
              لوحة التحكم
            </p>
          </div>
        </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-arabic text-[11px] leading-none mt-0.5 opacity-60">
                  {item.arabic}
                </p>
              </div>
      ...
      <div className="px-3 py-3 border-t border-ink-800">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-ink-800/50 mb-1">
          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-gold-400 to-gold-700
            flex items-center justify-center flex-shrink-0">
        ...
        <button onClick={logout}
          className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm
            text-red-400 hover:bg-red-950/30 transition-colors font-body">
  ...
  return (
    <div className="flex min-h-[calc(100dvh-56px)] bg-ink-50">
      <aside className="hidden md:flex flex-col w-56 bg-ink-950 border-r border-ink-800
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-64 bg-ink-950 border-r border-ink-800
            flex flex-col h-full">
            <button onClick={() => setSidebarOpen(false)}
              className="absolute top-3 right-3 text-ink-400 hover:text-ivory p-1.5
                rounded-lg hover:bg-ink-800 transition-colors">
              <X className="w-5 h-5" />
            </button>
            <SidebarContent />
          </aside>
      ...
      <div className="md:hidden fixed top-14 left-0 right-0 z-40 bg-ink-900 border-b
        border-ink-800 px-4 h-11 flex items-center gap-3">
        <button onClick={() => setSidebarOpen(true)}
          className="text-ink-400 hover:text-ivory p-1.5 rounded-lg hover:bg-ink-800
            transition-colors">
