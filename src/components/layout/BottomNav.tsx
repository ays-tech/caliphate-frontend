...
import { Home, BookOpen, Users, LayoutDashboard, LogIn, Info } from 'lucide-react';
...
export default function BottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 safe-bottom">
      ...
      <div className="bg-ink-950/95 backdrop-blur-md px-1 pt-2 pb-3 flex items-center justify-around">
        ...
        {/* Admin or Sign In as the 5th slot */}
        {isAdmin ? (
          <Link
            href="/admin"
            className="flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition-all duration-200 min-w-[48px]"
          >
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-200 ${
              ...
              <LayoutDashboard className={`w-4 h-4 ${isActive('/admin') ? 'text-white' : 'text-ink-400'}`} strokeWidth={2} />
            </div>
